...
<body>
    ...
    <div class="cart-container">
        ...
        <div class="cart-summary">
            ...
            <p>Total Amount: ₹ <span id="total-amount"><?php echo number_format($total_amount, 2); ?></span></p>
        </div>

        <div class="check-out-div">
            <a href="" class='check-out-btn'>Proceed to Buy</a>
        </div>
    </div>
    ...
    <script>
        $(document).on("click", ".quantity-btn", function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (result.success) {
                        // Update total price for this item in the cart
                        let newTotalPrice = quantity * parseFloat(result.product_price); // Assuming product_price is returned in the response
                        $("#total_" + product_id).text("Total : ₹ " + newTotalPrice.toFixed(2));
                        updateCartSummary(); // Update the cart summary after quantity change
                    } else {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (result.success) {
                        ...
                        updateCartSummary(); // Update the cart summary after item removal
                    } else {
        ...
        function updateCartSummary() {
            ...
            // Iterate over each cart item to calculate totals
            $(".cart-item").each(function() {
                let quantity = parseInt($(this).find('.quantity-input').val());
                let productTotalPrice = parseFloat($(this).find('.total-price').text().replace("Total : ₹ ", ""));
                
                totalQuantity += quantity;
                totalAmount += productTotalPrice; // Accumulate total amount for each product
            });
